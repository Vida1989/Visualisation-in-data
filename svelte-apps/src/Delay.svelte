<script>
  ...
  let filteredPlantKeys = getFilteredPlantKeys(selectedYear);
  ...
  afterUpdate(() => {
    filteredPlantKeys = getFilteredPlantKeys(selectedYear);
  });

  function getFilteredPlantKeys(year) {
...
<div>
  ...
  <select bind:value={selectedPlantKey}>
    {#each filteredPlantKeys as plantKey}
      <option value={plantKey}>Distribution Plant: {plantKey}</option>
    {/each}
  </select>
